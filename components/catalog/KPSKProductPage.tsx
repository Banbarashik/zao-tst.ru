...
export default async function KPSKProductPage({ product }) {
  ...
  const productDesignText = {
    ksk: `Калорифер многоходовой ${product.shortName} выполнен в виде жесткого стального модуля с автономным гидравлическим трактом, включающим в себя конструкционные блоки подвода, распределения и отвода теплоносителя. Внутри корпуса размещен пакет теплопередающих элементов с накатным алюминиевым оребрением, зафиксированный в трубных решетках и соединенный с коллекторными крышками. Съемные боковые щитки с присоединительными отверстиями по периметру фланцев обеспечивают крепление теплообменника к воздуховодам вентиляционной системы. В комплексе с трубными решетками они образуют единый монтажный контур водяного воздухонагревателя ${product.shortName}.`,
    kpsk: `Калорифер одноходовой ${product.shortName} выполнен в виде жесткого стального каркаса прямоугольного сечения с трубными решетками и боковыми съемными щитками, распределительным и сборным коллекторами, патрубками и пакетом трубок с алюминиевым спирально- накатным оребрением, расположенных в шахматном порядке. Паровой воздухонагреватель должен эксплуатироваться: в системах отопления, вентиляции, кондиционирования с температурой теплоносителя не выше 130°С, давлением не более 0.3 МПа, теплоотдающие элементы изготовлены из стальных электросварных прямошовных труб; для технологического нагрева с температурой теплоносителя до 190°С, давлением до 1.2 МПа, теплоотдающие элементы изготовлены из бесшовных холоднодеформированных труб.`,
  };
    ...
    <div className="@container w-full lg:overflow-x-auto">
      ...
      <section className="mb-6">
        ...
        <ProductParagraph className="mb-4">
          ...
          присоединительные размеры {heatCarrierAdj.gen} теплообменника, диаметр
          условного прохода патрубков. По запросу воздухонагреватели могут
          изготавливаться с фланцами
          {engSeries === "ksk" && ", штуцерами с резьбовым соединением"} или с
          патрубками нестандартного диаметра, конфигурации и расположения.
        </ProductParagraph>
        <Image
          src={product.drawing}
      ...
      <section className="mb-10">
        ...
        <ProductParagraph className="mb-3">
          ...
          {engSeries === "ksk" &&
            `При компоновке нескольких калориферов в секции съемные боковые щитки демонтируются, теплообменники ${product.shortName} соединяются между собой и образуют единую фронтальную поверхность нагрева.`}
          {engSeries === "kpsk" &&
