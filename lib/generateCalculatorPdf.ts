...
export async function generateCalculatorPdf(
  ...
  const doc = createElement(PritochnyeCalculatorDocument, { state });
 ...
 *
 * @param state   Текущее состояние калькулятора
 * @param filename Имя файла без расширения (по умолчанию — из modelLabel)
 */
export async function downloadCalculatorPdf(
  state: CalculatorState,
  filename?: string,
): Promise<void> {
  const blob = await generateCalculatorPdf(state);
